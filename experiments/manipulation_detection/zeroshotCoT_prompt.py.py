...
def prediction(model, test_data):
    ...
    for idx, row in test_data.iterrows():
        count += 1
        logging.info(f"-----Running {model.model_id} zeroshotCoT prompting ({count}/{len(test_data)})-----")
        dialogue = row['Dialogue']
        pred = model.zeroshotCoT_prompting(dialogue)
        preds.append(pred)
    ...
    logging.info(f"\n----------{model.model_id} zeroshotCoT prompting result----------")
    logging.info(
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='zeroshotCoT')
